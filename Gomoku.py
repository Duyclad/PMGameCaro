...
def march(board,y,x,dy,dx,length):
   ...
   tim vi tri xa nhat trong dy, dx trong khoang length

    '''
    yf = y + length*dy 
    xf = x + length*dx
    #  chung nao yf, xf khong co trong board
...
def score_of_row(board,cordi,dy,dx,cordf,col):
    ...
    tra ve mot list voi moi phan tu dai dien cho so diem cua 5 khoi
    '''
    colscores = []
    y,x = cordi
    yf,xf = cordf
...
def score_of_col(board,col):
    ...
    tinh toan diem so moi huong cua column dung cho is_win
    '''
...
def TF34score(score3,score4):
    ...
    tra lai truong hop chac chan co the thang (4 o lien tiep)
    '''
    for key4 in score4:
        if score4[key4] >=1:
            for key3 in score3:
...
def stupid_score(board,col,anticol,y,x):
    '''
    co gang di chuyen y,x
    tra ve diem so tuong trung loi the
    '''
...
def winning_situation(sumcol):
    ...
    tra lai tinh huong chien thang dang nhu:
    {0: {}, 1: {(0, 1): 4, (-1, 1): 3, (1, 0): 4, (1, 1): 4}, 2: {}, 3: {}, 4: {}, 5: {}, -1: {}}
    1-5  luu diem co do nguy hiem tu thap den cao
    -1 la roi vao trang thai kem, can phong thu
    '''
...
def best_move(board,col):
    ...
    tra lai diem so cua mang trong loi the cua tung mau
    '''
    if col == 'w':
        anticol = 'b'
    else:
    ...
    movecol = (0,0)
    maxscorecol = ''
    # kiem tra neu ban co rong thi cho vi tri random neu khong thi dua ra gia tri tren ban co nen di 
    if is_empty(board):
